...
<template>
    ...
    <v-dialog v-model="dialog" persistent activator="parent" width="400px">
        <v-card>
            <h2 class="popup-title">{{ $t("DeleteSession.title") }}</h2>
            <v-divider></v-divider>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn :text="$t('DeleteShow.buttonCancel')" @click="dialog = false" class="button-form--actions"></v-btn>
                <v-btn @click="deleteSession"  color="primary" :text="$t('DeleteShow.buttonDelete')" variant="tonal" class="button-form--actions"></v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
